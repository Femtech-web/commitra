...
async function runPrepareCommitMsgHook(messageFilePath, _commitSource) {
  if (!messageFilePath) {
    console.error(
      chalk.red(
  ...
  try {
    const existing = await fs.readFile(messageFilePath, "utf8");
...
export { BASE_EXCLUDE_PATTERNS as B, LIBRARY_MAP as L, ROOT_MARKERS as R, TECH_MAP as T, buildCommitPrompt as a, buildEnhancedDiffContext as b, createAIClient as c, getRuntimeConfig as g, runPrepareCommitMsgHook as r };
