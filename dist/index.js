...
import { B as BASE_EXCLUDE_PATTERNS, R as ROOT_MARKERS, b as buildEnhancedDiffContext, g as getRuntimeConfig, a as buildCommitPrompt, c as createAIClient, T as TECH_MAP, L as LIBRARY_MAP, l as loadPackageJson } from './diff-DZJcZj9c.js';
...
`));
  ...
  console.error(chalk.redBright(`\u{1F4A5} Unexpected error:`));
...
function parsePyproject(root) {
  const file = path.join(root, "pyproject.toml");
  if (!exists(file)) return null;
  const py = toml.parse(read(file));
...
function parseRequirements(root) {
  ...
  const dependencies = lines.filter(Boolean).map((line) => {
...
function parseGoMod(root) {
  ...
  const dependencies = depMatches.map((m) => ({
    name: m[1],
    version: m[2]
  }));
  return {
...
function parseCargo(root) {
  ...
  if (!exists(file)) return null;
  const data = toml.parse(read(file));
...
function parseGemfile(root) {
  const file = path.join(root, "Gemfile");
  if (!exists(file)) return null;
  const lines = read(file).split("\n");
  const dependencies = lines.filter((l) => l.trim().startsWith("gem ")).map((l) => {
...
async function parseCsproj(root) {
  ...
  const deps = xml.Project.ItemGroup?.flatMap(
    (grp) => grp.PackageReference?.map((ref) => ({
      name: ref.$.Include,
      version: ref.$.Version
    })) || []
  ) || [];
...
async function detectProjectMetadata(root) {
  const detectors = [
    parsePackageJson,
    parsePyproject,
    parseRequirements,
  ...
  for (const detect of detectors) {
    const result = detect.length === 1 ? detect(root) : await detect(root);
...
function createPairs(schema, iterable, ctx) {
  const { replacer } = ctx;
  const pairs2 = new YAMLSeq(schema);
...
function findScalarTagByTest({ atKey, directives, schema }, value, token, onError) {
  const tag = schema.tags.find((tag2) => (tag2.default === true || atKey && tag2.default === "key") && tag2.test?.test(value)) || schema[SCALAR$1];
  if (schema.compat) {
    const compat = schema.compat.find((tag2) => tag2.default && tag2.test?.test(value)) ?? schema[SCALAR$1];
...
function directoryHasRootMarker(dir) {
...
function loadCommitraConfig(root) {
  const CONFIG_FILES = [
    ".commitra.json",
    ".commitra.yaml",
    ".commitra.yml",
    ...
    if (fs.existsSync(full)) {
      try {
        if (file.endsWith(".yaml") || file.endsWith(".yml")) {
          return require$$0.parse(fs.readFileSync(full, "utf-8"));
        }
...
function readDirectoryTree(root, depth = 3, excludeList = []) {
  const visited = /* @__PURE__ */ new Set();
  function isExcluded(item, fullPath) {
    return excludeList.some(
      (pattern) => minimatch(item, pattern, { matchBase: true }) || minimatch(fullPath, pattern, { matchBase: true })
    );
  }
...
function buildExcludeList(root) {
  ...
  return [.../* @__PURE__ */ new Set([...finalExcludes, ...userExcludes])];
}

async function runCommitCommand(options) {
  ...
  const dependencies = (meta.dependencies || []).map((d) => d.name).slice(0, 10).join(", ");
  ...
  const userLocale = cfg.locale || Intl.DateTimeFormat().resolvedOptions().locale.split("-")[0];
  ...
  const msgs = [
    {
      role: "system",
      content: "You are Commitra, an expert AI that generates clean, conventional-style commit messages."
    },
...
`,
    ...
    if (choice === "edit") {
      const edited = await text({
        ...
        initialValue: final,
        validate: (v) => v && v.trim().length > 0 ? void 0 : "Message cannot be empty."
      });
      if (isCancel(edited)) {
        outro("Commit cancelled.");
        process.exit(0);
...
function registerCommitCommand(program) {
  program.command("commit").description("Generate an AI-powered commit message").option("--suggest-only", "Print suggestion only").option("-g, --generate <n>", "Generate N suggestions", "1").action(runCommitCommand);
}
...
function groupTechStack(items) {
  ...
  for (const item of items || []) {
    ...
    const match = (list) => list.some((k) => lower.includes(k));
...
function buildReadmeFromJSON(data) {
  const unescapeNewlines = (text = "") => text.replace(/\\n/g, "\n");
  const section = (title, body) => body?.trim() && body.trim() !== "Not provided" ? `## ${title}

${body.trim()}

` : "";
  const list = (arr, formatter) => arr && arr.length ? arr.map(formatter).join("\n") + "\n\n" : "";
  const numbered = (arr) => arr && arr.length ? arr.map((s, i) => `${i + 1}. ${s}`).join("\n") + "\n\n" : "";
  const apiSection = (routes) => routes && routes.length ? routes.map(
    (r) => `
        ### \`${r.route}\`
...
${section(
    "Libraries & Utilities",
    data.libraries?.length ? data.libraries.map((l) => `- ${l.name}`).join("\n") + "\n\n" : ""
  )}
...
function buildApiDocFromJSON(data) {
  const list = (arr, fm) => arr && arr.length ? arr.map(fm).join("\n\n") + "\n" : "";
  return `
# ${data.title}
...
**Errors:**  
${r.errors?.length ? r.errors.map((x) => `- ${x}`).join("\n") : "Not provided"}
...
function detectLanguageAndManager(projectRoot) {
  ...
  return { language: "Unknown", packageManager: void 0 };
}
async function extractDependencies(projectRoot, language, packageManager) {
  try {
...
function addToCategory(out, category, label) {
  switch (category) {
    case "framework":
    ...
    case "database":
      ...
      break;
    case "ui":
      out.ui = Array.from(/* @__PURE__ */ new Set([...out.ui || [], label]));
      break;
    case "devops":
      out.devops = Array.from(/* @__PURE__ */ new Set([...out.devops || [], label]));
      break;
    case "testing":
      out.testing = Array.from(/* @__PURE__ */ new Set([...out.testing || [], label]));
      break;
    case "cloud":
      out.cloud = Array.from(/* @__PURE__ */ new Set([...out.cloud || [], label]));
      break;
    case "libraries":
      out.libraries = Array.from(/* @__PURE__ */ new Set([...out.libraries || [], label]));
      break;
    case "buildTools":
      out.buildTools = Array.from(/* @__PURE__ */ new Set([...out.buildTools || [], label]));
      break;
    default:
      out.libraries = Array.from(/* @__PURE__ */ new Set([...out.libraries || [], label]));
  }
...
async function detectStack(projectRoot) {
  ...
  const local = classifyLocally(deps);
  ...
  if (localConfidence >= 0.8) {
    ...
    try {
      const ai = await classifyWithAI({ language, packageManager, dependencies: deps });
...
function synthesizeStackInfo(language, packageManager, local, source) {
  const summaryParts = [];
...
function mergeLocalAndAI(language, packageManager, local, ai) {
  const weightLocal = 0.7;
  const weightAI = 0.3;
  const pickString = (l, a) => l && a ? Math.random() < 0.5 ? l : a : l || a || null;
  const merged = {
    language,
    packageManager,
    ...
    ui: Array.from(/* @__PURE__ */ new Set([...local.ui || [], ...ai.ui || []])),
    devops: Array.from(/* @__PURE__ */ new Set([...local.devops || [], ...ai.devops || []])),
    libraries: Array.from(/* @__PURE__ */ new Set([...local.libraries || [], ...ai.libraries || []])),
    testing: Array.from(/* @__PURE__ */ new Set([...local.testing || [], ...ai.testing || []])),
    cloud: Array.from(/* @__PURE__ */ new Set([...local.cloud || [], ...ai.cloud || []])),
    buildTools: Array.from(/* @__PURE__ */ new Set([...local.buildTools || [], ...ai.buildTools || []])),
...
function detectAPIRoutes(root, language) {
...
function scanAllForApi(dir, output, base) {
...
function collectRoutes(dir, output, base) {
...
function cleanRoutePath(fullPath, base) {
  const rel = fullPath.replace(base, "").replace(/\\/g, "/");
  return rel.replace(/\.(ts|js|py|go|rs|php|java|rb|swift|rb)$/i, "").replace(/\/route$/, "").replace(/\/index$/, "").replace(/controller/i, "").trim();
}
...
async function buildReadmeContext(root) {
...
async function buildApiContext(root) {
  const meta = await detectProjectMetadata(root);
...
async function buildDiagramContext(root, depth = 3) {
  const meta = await detectProjectMetadata(root);
  const name = String(meta.name || path.basename(root));
  const description = String(meta.description || "An app analyzed by Commitra.");
  const exclude = buildExcludeList(root);
...
const runReadmeCommand = async (options) => {
  console.log(chalk.cyanBright("\n\u{1F4D8} Commitra README Generator\n"));
  ...
  try {
    ...
    const response = await ai.chat(
      ...
      { max_tokens: 16e3, temperature: 0.4 }
    );
...
function registerReadmeCommand(program) {
  program.command("readme").description("Generate a signature-style README.md using AI").option("-o, --output <file>", "Specify output file (default: README.md)").action((options) => runReadmeCommand(options));
}
...
function buildApiPrompt({ name, stack, apis, structure }, baseUrl) {
  return `
...
async function runApiCommand(options) {
  console.log(chalk.cyanBright("\n\u{1F4E1} Commitra API Documentation Generator\n"));
  ...
  try {
    ...
    const response = await ai.chat(
      ...
      { max_tokens: 16e3, temperature: 0.2 }
    );
    const raw = response.choices?.[0]?.message?.content || "{}";
    let json;
    try {
...
function registerApiCommand(program) {
  program.command("api").description("Generate full API documentation using AI").option("-o, --output <file>", "Specify output file (default: API_DOCS.md)").option("-b, --base-url <url>", "Specify the API base URL (optional)").action((options) => runApiCommand(options));
}
...
`;
function buildDiagramUserPrompt(ctx) {
  return `
...
function mermaidFromDiagramJson(data, type) {
...
function normalizeLabel(label) {
  return label.replace(/\[([^\]]+)\]/g, ":$1").replace(/\[/g, "(").replace(/\]/g, ")");
}
function cleanMermaidLabel(label) {
  return normalizeLabel(label).replace(/^\/+/, "");
}
function safeId(id) {
  return id.replace(/\W+/g, "_").replace(/^_+/, "").replace(/_+$/, "");
}
function fallbackMermaidFromContext(ctx, type) {
  const nodes = [];
  nodes.push({
    id: safeId("Client"),
  ...
  const apiNodes = (ctx.apis || []).map((r, i) => {
  ...
  if (type === "sequence") {
    ...
    if (hasContracts) lines2.push("participant CH as Blockchain");
    lines2.push("U->>FE: User action");
    if (apiNodes.length) {
      lines2.push("FE->>API: call endpoint");
      lines2.push("API->>SRV: delegate to service");
    } else {
      lines2.push("FE->>SRV: call service");
    }
    if (hasContracts) {
      lines2.push("SRV->>CH: write tx");
      lines2.push("CH-->>SRV: tx receipt");
    } else {
      lines2.push("SRV->>Database: store/read");
      lines2.push("Database-->>SRV: result");
    }
    lines2.push("SRV-->>API: response");
    lines2.push("API-->>FE: JSON");
    lines2.push("FE-->>U: UI update");
    return lines2.join("\n");
  }
  if (type === "system") {
    const lines2 = [
    ...
    if (hasContracts) lines2.push(`BC["Contracts / ABI"]`);
    lines2.push("end");
    lines2.push("FE --> A1");
    lines2.push("A1 --> S1");
    if (hasContracts) lines2.push("S1 --> BC");
    lines2.push("S1 --> DB");
    return lines2.join("\n");
  }
  const lines = ["flowchart TD", "Client --> Frontend"];
  apiNodes.forEach((id) => lines.push(`Frontend --> ${id}`));
...
async function runDiagramCommand(opts) {
  console.log(chalk.cyanBright("\n\u{1F578}\uFE0F  Commitra Workflow Diagram Generator\n"));
  const spinner = ora("Scanning project and building context...").start();
  try {
    const root = getProjectRoot();
    const depth = Number(opts?.depth ?? 3);
    const type = (opts?.type || "flow").toLowerCase();
...
function registerDiagramCommand(program) {
  program.command("diagram").description("Generate an architecture Mermaid diagram (AI-powered)").option("-s, --summarize", "Include an AI-generated short architecture summary (best-effort)").option("-o, --output <file>", "Save diagram to a file (default: FLOW.md)").option("-d, --depth <n>", "Directory tree depth to scan", (v) => parseInt(v, 10)).option("-t, --type <type>", "Diagram type: flow | sequence | system (default: flow)", "flow").option("-b, --base-url <url>", "Optional base URL used when rendering example endpoints").action((options) => runDiagramCommand(options));
}

async function runProjectFolderCommand(options) {
  console.log(chalk.cyanBright("\n\u{1F4C2} Commitra Project Folder generator\n"));
...
function registerProjectFolderCommand(program) {
  program.command("folder").description("Visualize the project directory tree structure").option("-d, --depth <number>", "Depth level to scan (default: 3)", (val) => parseInt(val, 10)).option("-o, --output <file>", "Save folder map to a Markdown file (default: PROJECT_FOLDER.md)").action((options) => runProjectFolderCommand(options));
}

const HOOK_NAME = "prepare-commit-msg";
const REL_HOOK_PATH = `.git/hooks/${HOOK_NAME}`;
async function installHook(hookPath, hookEntryFile) {
  const exists = await fs$1.stat(hookEntryFile).then(() => true).catch(() => false);
  if (!exists) {
    console.error(
      chalk.red(
...
Run \`npm run build\` first.`
  ...
  await fs$1.mkdir(path.dirname(hookPath), { recursive: true });
  const hookExists = await fs$1.readFile(hookPath, "utf8").then((c) => c.includes("Commitra Git hook")).catch(() => false);
  if (hookExists) {
  ...
  `;
  await fs$1.writeFile(hookPath, shim);
  await fs$1.chmod(hookPath, 493);
  console.log(chalk.green(`\u2714 Commitra hook installed \u2192 ${hookPath}`));
}
async function uninstallHook(hookPath) {
  const exists = await fs$1.readFile(hookPath, "utf8").then((c) => c.includes("Commitra Git hook")).catch(() => false);
  if (!exists) {
    console.log(chalk.gray("Hook is not installed or was not created by Commitra."));
    return;
  }
  await fs$1.unlink(hookPath);
  console.log(chalk.yellow(`\u{1F5D1}\uFE0F Commitra hook removed \u2192 ${hookPath}`));
}
function registerHookCommand(program) {
  program.command("hook").description("Install or uninstall Commitra\u2019s prepare-commit-msg git hook").argument("<action>", "install | uninstall").action(async (action) => {
    const root = getProjectRoot();
...
function handleGet(keys) {
  const cfg = ensureConfigFile();
  if (!keys.length) {
    console.log(chalk.cyanBright("\n\u{1F9FE} Current Commitra Config:\n"));
...
function registerConfigCommand(program) {
  ...
  cmd.command("get [keys...]").description("Get one or more configuration values").action((keys) => handleGet(keys));
  cmd.command("set [pairs...]").description("Set one or more configuration values (key=value)").action((pairs) => handleSet(pairs));
}
...
async function runCli(argv = process.argv) {
  ...
  showBanner();
  program.name("commitra").description("Commitra - the intelligent commit & project AI CLI").version(packageJson.version).showHelpAfterError().configureHelp({
